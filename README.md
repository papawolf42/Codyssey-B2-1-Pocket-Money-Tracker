...
- **구현 방식**: `storage.py`의 `read_jsonl()` 및 `TransactionRepository.get_all()`은 파일 전체를 한 번에 메모리에 적재하지 않고, `for line in f:` 순회하며 `yield Transaction.from_dict(...)`를 통해 레코드를 1건씩 스트리밍합니다.
...
## 6. 과제를 진행하며 학습하고 고민한 내용들

본 가계부 프로젝트를 구현하며 직접 부딪히고 해결했던 핵심 개념과 원리들입니다:

### 1) 파이썬 기본기 및 데이터 모델
- **한글 저장 시 깨짐 방지 (`ensure_ascii=False`)**: JSON 저장 시 기본값(`True`)을 그대로 쓰면 한글이 `\uac00` 같은 유니코드 코드로 변환됩니다. `ensure_ascii=False`를 지정하여 파일에서도 사람이 읽을 수 있는 온전한 한글로 저장되게 했습니다.
- **가변 기본 인자의 덫 방지 (`default_factory=list`)**: Dataclass에서 `tags: list = []`로 기본값을 지정하면 모든 객체가 단 하나의 리스트 메모리를 공유하는 파이썬 고유의 버그가 생깁니다. 매번 독립된 새 리스트가 생성되도록 `field(default_factory=list)`를 사용했습니다.
- **타입 힌트와 실제 값 검증의 차이**: 타입 힌트(`amount: int`, `date: str`)는 개발자를 위한 가이드일 뿐 런타임 실행 오류를 막아주지 못합니다. 따라서 `__post_init__` 메서드를 통해 음수 금액이나 날짜 형식 오류를 실제 실행 시점에 엄격하게 검증하도록 설계했습니다.
- **딕셔너리 안전 누적 합산 (`dict.get(key, 0)`)**: 카테고리별 지출을 집계할 때 매번 `if key in dict`로 확인하지 않고, `expenses[cat] = expenses.get(cat, 0) + amount` 패턴을 적용하여 `KeyError` 없이 한 줄로 안전하게 합산했습니다.
- **날짜 문자열 파싱과 역법 검증 (`strptime` vs `strftime`)**: 사용자가 입력한 문자열이 2월 30일이나 13월처럼 존재하지 않는 날짜인지 `datetime.strptime()`의 역법 달력 검증 기능을 활용해 정확히 걸러내고, 출력 시에는 `strftime()`으로 포맷을 통일했습니다.

### 2) 파일 입출력 및 메모리 최적화
- **대용량 처리를 위한 JSON 대신 JSONL 선택**: 일반 JSON은 파일 전체가 하나의 거대한 리스트(`[...]`)여서 매번 파일 전체를 메모리에 올려야 합니다. 반면 JSONL은 한 줄마다 독립된 JSON 객체이므로 엔터(`\n`) 단위로 한 줄씩 읽고 쓸 수 있어 대용량 데이터 처리에 훨씬 유리합니다.
- **`yield` 제너레이터 스트리밍을 통한 메모리 절약**: 파일 전체를 한꺼번에 메모리에 올리지 않고, `yield`를 사용해 필요한 순간에만 한 줄씩 읽고 처리하여 데이터 건수가 많아져도 메모리 사용량을 O(1) 상수로 일정하게 유지했습니다.

### 3) CLI 설계 및 사용자 입력 검증
- **대화형 즉시 검증과 옵션 일괄 검증의 역할 분리**: 대화형 `add` 명령은 6개 질문을 끝까지 다 입력받은 뒤에 첫 번째 날짜가 틀렸다고 알려주면 사용자의 헛수고가 되므로, `input()` 직후 즉시 오류를 알리도록 설계했습니다. 반면 `search`나 `update` 같은 옵션 인자 명령은 Service 계층에서 일괄 검증하도록 입력 방식별 역할을 명확히 분리했습니다.
- **하위 명령어 계층화 (`argparse` subparsers)**: `git`처럼 `add`, `list`, `budget`, `summary` 등 각 기능별로 독립된 하위 파서를 구성하여, 명령어마다 필요한 고유 옵션만 직관적으로 연결되도록 구조화했습니다.
